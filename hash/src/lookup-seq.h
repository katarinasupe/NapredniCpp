...
class HashTable{
    ...
    public:
        ...
        {
                for( Bucket vector : mBuckets ){
                    vector.print_bucket_value(out);
                }
        }
        ...
        std::optional<BucketValue> find_if(Predicat p) const
        {
            ...
            for( Bucket vector : mBuckets ){
                optional_find = vector.find_if_bucket(p);
        ...
        std::list<BucketValue> find_all_if(Predicat p) const
        {
            ...
            for( Bucket vector : mBuckets ){
                if(!vector.find_all_if_bucket(p).empty())
                {
                    for( BucketValue pair : vector.find_all_if_bucket(p) ){
                        list_of_bucket_values->push_back(pair);
                    }
                }    
            }
